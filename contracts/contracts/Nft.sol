...
contract PriceTureNFT is ERC721, ERC721URIStorage, KeeperCompatible, Ownable {
   ...
   // Mapping tokenId => currentPriceTier => priceThreshold
    mapping(uint256 => mapping(uint256 => uint256)) public _priceTiersThresholds;
    ...
    function updateTokenURI() public {
     ...
     function determinePriceTier(int256 latestPrice, uint256 tokenId) public view returns (uint256) {
    ...
    function getChainlinkDataFeedLatestAnswer(uint256 tokenId) public view returns (int) {
        (
            ,
            int price,
            ,
            ,
        ) = priceFeeds[tokenId].latestRoundData();
        return price;
    }
